...
class App:
    def __init__(self):
        ...
        self.main_pdf = None
        self.po_pdf = None
        self.roster_xlsx = None

        pad = 8
        tk.Label(self.root, text="Choose inputs, then click Run.", font=("Segoe UI", 11, "bold")).pack(pady=12)

        frm = tk.Frame(self.root)
        frm.pack(padx=pad, pady=pad, fill="x")

        # Buttons + labels
        self.btn_main = tk.Button(frm, text="Select Main Staffing PDF (RN/CNA/HUC)", width=38, command=self.pick_main_pdf)
        self.btn_main.grid(row=0, column=0, sticky="w")
        self.lbl_main = tk.Label(frm, text="No file selected", fg="#666")
        self.lbl_main.grid(row=1, column=0, sticky="w", pady=(0, pad))

        self.btn_po = tk.Button(frm, text="Select PO Staffing PDF", width=38, command=self.pick_po_pdf)
        self.btn_po.grid(row=2, column=0, sticky="w")
        self.lbl_po = tk.Label(frm, text="No file selected", fg="#666")
        self.lbl_po.grid(row=3, column=0, sticky="w", pady=(0, pad))

        self.btn_roster = tk.Button(frm, text="Select Staffing Bible (Excel)", width=38, command=self.pick_roster)
        self.btn_roster.grid(row=4, column=0, sticky="w")
        self.lbl_roster = tk.Label(frm, text="No file selected", fg="#666")
        self.lbl_roster.grid(row=5, column=0, sticky="w", pady=(0, pad))

        self.btn_run = tk.Button(self.root, text="Run & Save Output", height=2, width=32, command=self.run_all)
        self.btn_run.pack(pady=16)

        tk.Label(
            self.root,
            text="Tips:\n• PDFs can be selected in any order.\n• Staffing Bible should be your OneDrive-synced SharePoint copy.\n• Output is one Excel grouped by ROLE → SHIFT with color coding.",
            justify="left", fg="#555"
        ).pack(padx=pad, anchor="w")

    def pick_main_pdf(self):
        path = filedialog.askopenfilename(title="Select Main Staffing PDF (RN/CNA/HUC)",
                                          filetypes=[("PDF files", "*.pdf"), ("All files", "*.*")])
        if path:
            self.main_pdf = path
            self.lbl_main.config(text=os.path.basename(path))

    def pick_po_pdf(self):
        path = filedialog.askopenfilename(title="Select PO Staffing PDF",
                                          filetypes=[("PDF files", "*.pdf"), ("All files", "*.*")])
        if path:
            self.po_pdf = path
            self.lbl_po.config(text=os.path.basename(path))

    def pick_roster(self):
        path = filedialog.askopenfilename(title="Select Staffing Bible (Excel)",
                                          filetypes=[("Excel", "*.xlsx;*.xlsm"), ("All files", "*.*")])
        if path:
            self.roster_xlsx = path
            self.lbl_roster.config(text=os.path.basename(path))
    ...
    def run_all(self):
        ...
        if not self.main_pdf:
            messagebox.showwarning(APP_TITLE, "Please select the Main Staffing PDF (RN/CNA/HUC).")
            return
        if not self.po_pdf:
            messagebox.showwarning(APP_TITLE, "Please select the PO Staffing PDF.")
            return
        if not self.roster_xlsx:
            messagebox.showwarning(APP_TITLE, "Please select the Staffing Bible (Excel).")
            return

        try:
            df_main = parse_pdf_to_df(self.main_pdf)
            df_po = parse_pdf_to_df(self.po_pdf)
            df_all = pd.concat([df_main, df_po], ignore_index=True)
        except Exception as e:
            messagebox.showerror(APP_TITLE, f"Failed to parse PDFs:\n{e}\n\n{traceback.format_exc()}")
            return

        try:
            # Read roster's "CRT STAFFING" and ensure a D_NAME column from its 4th column (D)
            df_roster = pd.read_excel(self.roster_xlsx, sheet_name="CRT STAFFING")
            ...
            messagebox.showerror(APP_TITLE, f"Failed to read Staffing Bible:\n{e}\n\n{traceback.format_exc()}")
            return

        try:
            matched = match_pdf_to_roster(df_all, df_roster, roster_name_col="D_NAME", coarse_threshold=85)
            if matched.empty:
                messagebox.showwarning(APP_TITLE, "No matches found.\nCheck the name column (D) and the PDFs.")
                return
        except Exception as e:
            messagebox.showerror(APP_TITLE, f"Matching failed:\n{e}\n\n{traceback.format_exc()}")
            return

        # ---- Build unmatched list for QA ----
        try:
            pdf_people = df_all[~df_all["NAME"].str.contains("<OPEN SHIFT>", case=False, na=False)][
                ["SHIFT", "ROLE", "NAME"]
            ].copy()
        ...
        base_out = "MATCHED_" + os.path.splitext(os.path.basename(self.main_pdf))[0] + "_PLUS_PO.xlsx"
        save_path = filedialog.asksaveasfilename(
            title="Save color-coded Excel",
        ...
        try:
            out_path = write_grouped_workbook(
                matched,
                self.roster_xlsx,
                roster_sheet="CRT STAFFING",
                out_path=save_path,
            ...
            messagebox.showerror(APP_TITLE, f"Failed to write Excel:\n{e}\n\n{traceback.format_exc()}")
